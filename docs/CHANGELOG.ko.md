...
- **Qt6 데스크톱 앱이 Windows 11 설정 스타일 셸로 바뀌어 실행 중인 Windows pod 제어를 한곳에 모았습니다.** Dashboard가 첫 화면이며 pod 상태와 시작/중지 제어, RAM·CPU·디스크 링, 빠른 작업, 실행 중인 세션, 고정 앱 타일, reverse-open 스위치를 제공합니다. Applications는 검색 가능한 타일 또는 목록과 범주별 개수, 컨텍스트 작업으로 발견된 시작 메뉴 앱을 표시합니다. Settings는 연결, 하드웨어, 업데이트, 통합, 지역화, 위험 작업을 그룹으로 정리하고 변경 내용을 저장하기 전까지 헤더에 변경 표시를 냅니다. Tools, Terminal, Info, Devices, License가 나머지 페이지를 구성하며, Devices는 USB와 PCI 장치를 그룹화하고 할당 개수를 보여 주며 필터와 위험한 PCI 할당 표시를 제공합니다.

- **프로비저닝 진행 상태가 컨테이너 로그보다 먼저 dockur의 `msg.html` 상태 엔드포인트를 읽습니다** (#863). CLI와 GUI는 버퍼링된 로그 출력을 기다리지 않고도 현재 업스트림 단계를 보여 줄 수 있으며, HTTP 상태 페이지를 사용할 수 없을 때는 기존 로그 기반 경로를 유지합니다.

### Changed

- **데스크톱 셸이 고정 실행기 레이아웃을 강제하지 않고 화면 공간에 맞춰 탐색과 창 장식을 조정합니다.** 일반 너비에서는 320px 탐색 창을 표시하고, 1100px 아래에서는 48px 아이콘 레일로 바뀝니다. 확장한 창은 페이지 위에 겹쳐 표시되며 바깥을 클릭하면 닫힙니다. 사용자 지정 제목 표시줄은 최소화, 최대화, 닫기, 끌기, 더블 클릭 최대화를 제공합니다. `WINPODX_NATIVE_TITLEBAR=1`을 설정하면 시스템 창 장식으로 되돌릴 수 있습니다. Fluent 라이트 및 다크 테마는 운영 체제를 따르며, 임시로 `WINPODX_COLOR_SCHEME=light` 또는 `dark`를 지정할 수 있습니다. Segoe UI Variable과 Segoe UI가 없는 환경에서는 번들 Selawik 글꼴이 인터페이스 가독성을 유지합니다.

- **`winpodx launch`가 간결한 시작 메뉴 스타일 플라이아웃으로 열립니다** — 검색, 고정/최근 행, 전체 앱 그리드를 데스크톱 앱과 같은 스타일로 보여주고 동일한 라이트/다크 스킴을 따릅니다.

- **트레이 앱 실행 메뉴가 최신 상태를 유지하고 각 앱의 전체 실행 설정을 보존합니다.** 표시 가능한 Windows 앱을 실행 메뉴 등급과 이름을 기준으로 일관되게 정렬하고, 숨긴 앱은 제외하며, 기존 20개 앱 제한을 제거했습니다. 트레이 실행 시 launch URI, window-class 힌트, 기본 인자, 앱 아이콘 및 앱별 RDP 재정의를 모두 전달합니다. 중첩 메뉴 신호를 안정적으로 보내지 않는 KDE Plasma 같은 데스크톱에서도 동작하도록 메뉴를 열 때와 상태 타이머에서 모두 새로 고치며, 앱 목록을 읽지 못하면 마지막으로 정상 작동한 메뉴를 유지합니다. 정렬, 필터링, 실행 메타데이터, 액션 수명 및 실패 안전 메뉴 재구성을 동작 테스트로 검증합니다.
- **호스트 장치 목록에 유용한 PCI 이름과 패스스루 중심 정렬을 적용했습니다** (@silentone12725 기여, #819). PCI 항목은 안정적인 하드웨어 ID를 유지하면서 제조사/모델 이름, 번역된 장치 클래스, IOMMU 메타데이터를 함께 표시합니다. USB 주변 장치는 계속 먼저 표시하고, 그룹 유무와 관계없이 PCI 장치를 모두 유용성 기준으로 정렬하되 각 IOMMU 그룹은 인접 상태와 PCI 주소 순서를 유지합니다. CLI와 GUI가 같은 정렬 정책을 공유합니다.

### Removed

- **Python 3.9 지원을 제거했으며, 이제 WinPodX는 Python 3.10 이상이 필요합니다.** 데스크톱 앱 리디자인에는 현대적인 dataclass와 typing 동작이 필요하며, Python 3.9만 호환성 shim이 필요했던 유일한 지원 런타임이었습니다. `tomllib`는 Python 3.11부터 stdlib에 들어가므로 Python 3.10에서는 계속 `tomli`를 설치합니다. RHEL 9, AlmaLinux 9, Rocky Linux 9에서는 el9 패키지가 AppStream의 Python 3.11 스택을 함께 설치합니다.

### Fixed

- **대시보드 리소스 색상과 애플리케이션 레이블이 의미에 맞게 표시됩니다** (#811, #812; #819의 @silentone12725 기여, @GameSoul7Eugene 제보 감사). RAM은 보라색 강조색, CPU와 정상 디스크 사용량은 파란색을 사용하고, 디스크 사용량이 설정된 자동 확장 임계값에 도달하면 빨간색으로 전환됩니다. 같은 임계 상태를 색상에만 의존하지 않고 Qt 접근성 속성의 번역된 경고로도 제공합니다. 실행 화면 이름을 탐색 메뉴, 빈 상태 안내, 문서, 6개 번역 카탈로그 전체에서 "Applications"에 해당하는 표현으로 통일했습니다.
...
이번 변경들에 기여해 주신 @silentone12725에게 감사드립니다.
